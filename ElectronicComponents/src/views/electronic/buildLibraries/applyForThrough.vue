          ...
          <div class="scard" v-for="(item,index) in listData" :key="index">
            <a class="card">
              ...
              <div class="card-actions">
                <button type="button" class="btn btn-danger btn-sm" style="background:#409eff;border:1px solid #409eff;" @click="buildWarehouse(index)">
                ...
                <button type="button" class="btn btn-warning btn-sm" style="background:#fff;border:1px solid #409eff;color:#409eff" @click="lookDetail(index)">
                  <i class="icon icon-file-text-o"></i> 入库
                </button>
...
export default {
  ...
  methods: {
    lookDetail(index) {
      putInStorage({id: this.listData[index].id})
          .then(res => {
