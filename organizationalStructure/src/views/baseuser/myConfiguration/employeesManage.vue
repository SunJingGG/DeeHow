    ...
    <el-dialog title="部门" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="form" :rules="rules" ref="ruleForm" label-width="100px">
        ...
        <el-form-item label="上级部门" prop="parentId">
          <el-select v-model="form.parentId" style="width:90%" placeholder="上级部门" :disabled="true">
            <el-option v-for="(item,index) in sdeptList" :key="index" :label="item.deptName" :value="item.id"></el-option>
            <el-option label="无" value="0"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="部门负责人" prop="parentId">
          <el-select v-model="form.leadingOfficial" style="width:90%" placeholder="请添加部门负责人">
            <div class="search">
              <el-input class="searchinput" v-model="form.chargeman" placeholder="请输入负责人名字"></el-input>
              <img src="../../../assets/byProduct/search.png" alt="" @click="chooseUser()">
            </div>
            <el-option v-for="(item,index) in chargeList" :key="index" :label="item.userName" :value="item.id"></el-option>
            <el-option label="无" :value="'0'"></el-option>
          </el-select>
        </el-form-item>
        <span>部门负责人为部门管理者，数据系统、销售报表等均可按照设置后部门负责人统计</span>
        <br>
...
export default {
  ...
  methods: {
    ...
    chooseUser(){
      readUserList({userName:this.form.chargeman}).then(res =>{
...
// .zTreeDemoBackground .ztree ul li{
//   line-height:28px;
// }
.rightContent {
