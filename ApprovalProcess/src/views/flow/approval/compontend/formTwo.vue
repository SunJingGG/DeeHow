...
import { readAddApplyDetialByKey } from "@/api/ele";
export default {
  props: ["processId"],
  data() {
    return {
  ...
  methods: {
    ...
    readData() {
      readAddApplyDetialByKey({ putStorageProcessKey: this.processInstanceId })
        .then(res => {
          console.log(res);
          this.auditObj = res.data;
          this.initData();
          this.dataSheet = res.data.public.manual.value;
          this.$emit("childByValue", res.data.addApplyId);
          console.log(res.data.addApplyId);
        })
        .catch(err => {
          console.log(err);
        });
  ...
  mounted() {
    ...
    this.processInstanceId = this.processId
    this.readData();
  }
};
</script>
